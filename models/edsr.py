...
class EDSR12(nn.Module):
    def __init__(self, args, conv=default_conv):
        super(EDSR12, self).__init__()
        self.args = args
        n_resblocks = args.n_resblocks
        n_feats = args.n_feats
        kernel_size = 3
        ...
        # define head module
        m_head = [nn.Conv2d(
        args.n_colors, n_feats, kernel_size,
...
@register('edsr-baseline-space-to-depth')
def make_edsr_baseline(n_resblocks=16, n_feats=64, res_scale=1,
                       scale=2, no_upsampling=False, rgb_range=1,n_colors=12):
    args = Namespace()
    args.n_resblocks = n_resblocks
    args.n_feats = n_feats
    args.res_scale = res_scale
